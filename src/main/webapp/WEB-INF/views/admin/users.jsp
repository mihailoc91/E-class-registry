                ...
                <c:forEach items="${allUsersDto}" var="user">
                    <tr>
                        ...
                        <c:if test="${(user != null) && ( user.getStatus() ==3)}">
                            <td><a href="<c:url value="/admin/teacher?id=${user.getId()}"/>">Edit subjects</a></td> 
                        </c:if>
                        <td><a href="<c:url value="/admin/user?id=${user.getId()}&status=${user.getStatus()}"/>">Edit</a></td>
                        <td><a href="<c:url value="/admin/user/remove?id=${user.getJmbg()}&status=${user.getStatus()}"/>">Delete</a></td>
                    </tr>
                </c:forEach>
            </table>
        </div>
